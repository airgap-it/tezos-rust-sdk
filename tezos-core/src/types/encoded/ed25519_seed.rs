use crate::internal::coder::encoded::encoded_bytes_coder::EncodedBytesCoder;
use crate::internal::coder::Encoder;
use crate::types::encoded::{Encoded, MetaEncoded};
use crate::{Error, Result};

pub struct Ed25519Seed {
    base58: String,
}

impl Encoded for Ed25519Seed {
    fn base58(&self) -> &str {
        &self.base58
    }
    fn meta(&self) -> &MetaEncoded {
        &META
    }
    fn new(base58: String) -> Result<Self> {
        if META.is_valid_base58(&base58) {
            return Ok(Ed25519Seed { base58 });
        }
        return Err(Error::InvalidBase58EncodedData);
    }
}

pub const META: MetaEncoded = MetaEncoded {
    base58_prefix: "edsk",
    base58_length: 54,
    bytes_prefix: &[13, 15, 58, 7],
    bytes_length: 32,
};

impl TryFrom<&Vec<u8>> for Ed25519Seed {
    type Error = Error;

    fn try_from(value: &Vec<u8>) -> Result<Self> {
        let coder = EncodedBytesCoder::new();
        coder.decode_with_meta(value, &META)
    }
}

impl TryFrom<[u8; META.bytes_length]> for Ed25519Seed {
    type Error = Error;

    fn try_from(value: [u8; META.bytes_length]) -> Result<Self> {
        let coder = EncodedBytesCoder::new();
        coder.decode_with_meta(&value, &META)
    }
}

impl TryFrom<String> for Ed25519Seed {
    type Error = Error;

    fn try_from(value: String) -> Result<Self> {
        Ed25519Seed::new(value)
    }
}

impl TryFrom<&str> for Ed25519Seed {
    type Error = Error;

    fn try_from(value: &str) -> Result<Self> {
        Ed25519Seed::new(value.to_string())
    }
}

impl TryFrom<&Ed25519Seed> for Vec<u8> {
    type Error = Error;

    fn try_from(value: &Ed25519Seed) -> Result<Self> {
        let coder = EncodedBytesCoder::new();
        coder.encode(value)
    }
}
