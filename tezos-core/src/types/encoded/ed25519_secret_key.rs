use crate::internal::coder::encoded::encoded_bytes_coder::EncodedBytesCoder;
use crate::types::encoded::{Encoded, MetaEncoded};
use crate::{Error, Result};

#[derive(Debug)]
pub struct Ed25519SecretKey {
    base58: String,
}

impl Ed25519SecretKey {
    pub fn is_valid_base58(value: &str) -> bool {
        META.is_valid_base58(value)
    }

    pub fn is_valid_bytes(value: &[u8]) -> bool {
        META.is_valid_bytes(value)
    }
}

impl Encoded for Ed25519SecretKey {
    type Coder = EncodedBytesCoder;

    fn base58(&self) -> &str {
        &self.base58
    }
    fn meta(&self) -> &MetaEncoded {
        &META
    }
    fn new(base58: String) -> Result<Self> {
        if META.is_valid_base58(&base58) {
            return Ok(Ed25519SecretKey { base58 });
        }
        return Err(Error::InvalidBase58EncodedData);
    }
}

pub const META: MetaEncoded = MetaEncoded {
    base58_prefix: "edsk",
    base58_length: 98,
    bytes_prefix: &[43, 246, 78, 7],
    bytes_length: 64,
};

impl TryFrom<&Vec<u8>> for Ed25519SecretKey {
    type Error = Error;

    fn try_from(value: &Vec<u8>) -> Result<Self> {
        <Self as Encoded>::Coder::decode_with_meta(value, &META)
    }
}

impl TryFrom<[u8; META.bytes_length]> for Ed25519SecretKey {
    type Error = Error;

    fn try_from(value: [u8; META.bytes_length]) -> Result<Self> {
        <Self as Encoded>::Coder::decode_with_meta(&value, &META)
    }
}

impl TryFrom<String> for Ed25519SecretKey {
    type Error = Error;

    fn try_from(value: String) -> Result<Self> {
        Ed25519SecretKey::new(value)
    }
}

impl TryFrom<&str> for Ed25519SecretKey {
    type Error = Error;

    fn try_from(value: &str) -> Result<Self> {
        Ed25519SecretKey::new(value.to_string())
    }
}

impl TryFrom<&Ed25519SecretKey> for Vec<u8> {
    type Error = Error;

    fn try_from(value: &Ed25519SecretKey) -> Result<Self> {
        value.to_bytes()
    }
}
