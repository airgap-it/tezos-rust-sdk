use crate::internal::coder::encoded::encoded_bytes_coder::EncodedBytesCoder;
use crate::types::encoded::{Encoded, MetaEncoded};
use crate::{Error, Result};

pub struct ScriptExprHash {
    base58: String,
}

impl Encoded for ScriptExprHash {
    type Coder = EncodedBytesCoder;

    fn base58(&self) -> &str {
        &self.base58
    }
    fn meta(&self) -> &MetaEncoded {
        &META
    }
    fn new(base58: String) -> Result<Self> {
        if META.is_valid_base58(&base58) {
            return Ok(ScriptExprHash { base58 });
        }
        return Err(Error::InvalidBase58EncodedData);
    }
}

pub const META: MetaEncoded = MetaEncoded {
    base58_prefix: "expr",
    base58_length: 54,
    bytes_prefix: &[13, 44, 64, 27],
    bytes_length: 32,
};

impl TryFrom<&Vec<u8>> for ScriptExprHash {
    type Error = Error;

    fn try_from(value: &Vec<u8>) -> Result<Self> {
        <Self as Encoded>::Coder::decode_with_meta(value, &META)
    }
}

impl TryFrom<[u8; META.bytes_length]> for ScriptExprHash {
    type Error = Error;

    fn try_from(value: [u8; META.bytes_length]) -> Result<Self> {
        <Self as Encoded>::Coder::decode_with_meta(&value, &META)
    }
}

impl TryFrom<String> for ScriptExprHash {
    type Error = Error;

    fn try_from(value: String) -> Result<Self> {
        ScriptExprHash::new(value)
    }
}

impl TryFrom<&str> for ScriptExprHash {
    type Error = Error;

    fn try_from(value: &str) -> Result<Self> {
        ScriptExprHash::new(value.to_string())
    }
}

impl TryFrom<&ScriptExprHash> for Vec<u8> {
    type Error = Error;

    fn try_from(value: &ScriptExprHash) -> Result<Self> {
        value.to_bytes()
    }
}
